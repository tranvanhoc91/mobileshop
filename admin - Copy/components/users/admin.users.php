		...
		<table class="adminlist">
			...
			?>
            	<tr>
                    ...
                    <td>
                    <?php  $uid_login = getUserLoginedIn($user->id,$user->username,$_SERVER['REMOTE_ADDR']); ?>
                    </td>
...
/************************************************************************************/
	function getAllUser($s='',$status='',$active='',$gid=''){	//lay tat cac cac user de hien thi ra luoi du lieu
		...
		$query = "SELECT users.id,username,password,email,actived,block,registerDate,lastvisitDate,group_id
				  FROM users
				  ";
...
//Lay ve danh sach nhom user ->> tim kiem 
	...
	function getAllSelectGroupUser($uid='') {
		global $dbo;
		$dbo->setQuery("SELECT name,id FROM groups WHERE id <>0 ORDER BY id DESC ");
		$ugroup = $dbo->loadObjectList();
			echo '<select name="group_id">';
