...
class SiteDocument {
	...
	function setTitle($title=''){
		if ($title)
			$this->_title = $title.' - ' . Config::$title;
		else 
			$this->_title = Config::$sitename;
	}
	...
	function setModule(){
		//lay ve id cua user hien thoi
		$uo = new User();
		$uid = $uo->get();
		
		//lay ve nhom cua user
		if ($uid) {
		...
		$this->_dbo->setQuery("SELECT module,position
								FROM modules AS m
								LEFT JOIN groups AS g ON g.id = m.access
